...
from arc_dsl.models.pattern_completion.model import (
...
from arc_dsl.models.pattern_completion.training_data import (
    PatternCompletionDataset,
    bitmap_parametrized_dsl,
)
...
class PrintSampleTranslationsCallback(tf.keras.callbacks.Callback):
    def __init__(self, tokenizer: GridlispTokenizer):
        super().__init__()
        self.tokenizer = tokenizer

        rng = random.Random(42)
        static_validation_grids = [sample_multi_bitmap_grid(rng).grid for _ in range(3)]
        self.static_validation = [
            (
                grid.draw(),
                "".join([bitmap_parametrized_dsl(bmp, rng) for bmp in grid.bitmaps]),
                "".join([bmp.to_dsl() for bmp in grid.bitmaps]),
            )
            for grid in static_validation_grids
        ]

    def on_test_end(self, logs=None):
        tf.print()

        inference_model = PatternCompletionModel(
            model=self.model, tokenizer=self.tokenizer
        )

        for grid, paramd_dsl, dsl in self.static_validation:
            tf.print("=====")
            tf.print("Parametrized:", paramd_dsl)
            tf.print("Expected dsl:", dsl)

            completion = inference_model.complete(grid, paramd_dsl)

            if completion is None:
                tf.print("Pattern completion failed")
            else:
                spacer = np.full((grid.shape[0], 1), 5)
                stacked_grids = np.concatenate(
                    [grid, spacer, completion.draw()], axis=1
                )
...
train_ds = PatternCompletionDataset(sequence_length, batch_size).prefetch(
    tf.data.AUTOTUNE
)
val_ds = PatternCompletionDataset(sequence_length, batch_size).prefetch(
    tf.data.AUTOTUNE
)
...
transformer.fit(
    ...
    callbacks=[
        backup_callback,
        checkpoint_callback,
        PrintSampleTranslationsCallback(tokenizer),
